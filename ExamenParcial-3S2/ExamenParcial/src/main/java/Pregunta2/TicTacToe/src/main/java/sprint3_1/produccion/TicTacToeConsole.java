...
import sprint3_1.produccion.TicTacToeGame.Cell;
import sprint3_1.produccion.TicTacToeGame.GameState;
...
public class TicTacToeConsole {
    ...
    public void play() {
        ...
        while (!done) {
            ...
            System.out.print("Mueve la fila: ");
            row = in.nextInt();
            System.out.print("Mueve la columna: ");
            column = in.nextInt();
            if (row < 0 || row > game.getTotalRows() || column < 0 || column > game.getTotalColumns())
                System.out.println("Movimiento invalido (" + row + "," + column + ")");
            else {
